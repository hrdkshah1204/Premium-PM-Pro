...
const memberSteps = [
    {
        ...
        action: () => window.navToMember('m-sub1')
    },
    {
    ...
    },
    {
        ...
        action: () => window.navToMember('m-sub3')
    },
    {
        title: "4. Task History",
        text: "Completed tasks are safely archived here for your records.",
        action: () => window.navToMember('m-sub4')
    },
    {
        title: "5. My Profile",
        text: "Keep your contact info and emergency details up to date here.",
        action: () => window.navToMember('m-sub2')
    }
];

// --- UPDATED TOGGLE FUNCTION WITH SAFETY CHECK ---
window.toggleWalkthrough = () => {
    const modal = document.getElementById('walkthrough-modal');
    
    // Safety check: if the HTML isn't loaded yet, don't crash
    if (!modal) {
        console.warn("Walkthrough modal not found in HTML yet.");
        return;
    }

    modal.classList.toggle('hidden');
    if (!modal.classList.contains('hidden')) {
        currentStep = 0; // Always start at the beginning when opened
        renderStep();
    }
};
...
function renderStep() {
    ...
    // Auto-navigate the user to the correct page by safely executing the action function
    step.action();
}
